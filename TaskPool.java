...
public class TaskPool {
	...
	public void singleThreadTask() {
		...
		A = CalculateUtils.add(
				CalculateUtils.mult(alfa,
						CalculateUtils.mult(B, CalculateUtils.mult(MK, MO))),
				CalculateUtils.mult(CalculateUtils.min(Z),
						CalculateUtils.mult(E, MR)));
		...
		if (rank == 0 || (rank == k && notTwoCore)) {
			...
			if (rank == 0) {
				...
				if (notTwoCore) {
					...
					box[0] = message;
					// 2. Якщо rank = 0, то передати B, MK(k*H), MR(k*H), α.
					// Задачі знизу.
					MPI.COMM_WORLD.Isend(box, 0, 1, MPI.OBJECT, k,
							KEY.SEND_RECIVE_B_MK_MR_alfa_TO_k);
				} else {
				...
				MPI.COMM_WORLD.Isend(box, 0, 1, MPI.OBJECT, rightRank,
						KEY.SEND_RECIVE_B_MK_MR_alfa);

				box[0] = null;
			}
			if (rank == k && notTwoCore) {
				// 3. Якщо rank = k, то прийняти B, MK(k*H), MR(k*H), α, від
				// задачі зверху.
				MPI.COMM_WORLD.Recv(box, 0, 1, MPI.OBJECT, 0,
						KEY.SEND_RECIVE_B_MK_MR_alfa_TO_k);
			...
			MPI.COMM_WORLD.Recv(box, 0, 1, MPI.OBJECT, rightRank,
					KEY.SEND_RECIVE_Z_E_MO);
			E = box[0].getVector(0);
			Z = box[0].getVector(1);
			calcMinButtonTask();
			MO = box[0].getMatrix(0);
			box[0] = null;
			Request r = MPI.COMM_WORLD.Irecv(box, 0, 1, MPI.OBJECT, rightRank,
					KEY.SEND_RECIVE_RESULT_A);
			...
			if (rank == k && notTwoCore) {
				...
				MPI.COMM_WORLD.Send(box, 0, 1, MPI.OBJECT, 0,
						KEY.SEND_RECIVE_A_TO_0);
			}
			if (rank == 0) {
				if (notTwoCore) {
					box[0] = null;
					// 17. Якщо rank = 0, то Отримати MAH*k від задачі знизу
					MPI.COMM_WORLD.Recv(box, 0, 1, MPI.OBJECT, k,
							KEY.SEND_RECIVE_A_TO_0);
					A.merge(box[0].getVector(0));
				}
	...
	public void middleTaskGroup() {
		if (rank != 0 && rank != k && rank != endFirsRowRank && rank != P - 1) {
			...
			inputRequest[0] = MPI.COMM_WORLD.Irecv(box, 0, 1, MPI.OBJECT,
					leftRank, KEY.SEND_RECIVE_B_MK_MR_alfa);
			inputRequest[0].Wait();
			MK = box[0].getMatrix(0);
			MR = box[0].getMatrix(1);
			box[0].setMatrix(0, MK.copy(H, MK.size() - H));
			...
			inputRequest[1] = MPI.COMM_WORLD.Irecv(box, 0, 1, MPI.OBJECT,
					rightRank, KEY.SEND_RECIVE_Z_E_MO);
			inputRequest[1].Wait();
			Z = box[0].getVector(1);
			box[0].setVector(1, Z.copy(H, Z.size() - H));
			// 4. Передати MO, Z(Z.size()-1)*H, E задачі зліва.
			MPI.COMM_WORLD.Isend(box, 0, 1, MPI.OBJECT, leftRank,
					KEY.SEND_RECIVE_Z_E_MO);
			Z = Z.copy(0, H);
			calcMinButtonTask();
			E = box[0].getVector(0);
		...
		if (rank == P - 1 || (rank == endFirsRowRank && notTwoCore)) {
			...
			if (rank == endFirsRowRank) {
				...
				if (notTwoCore) {
					...
					box[0] = message;
					// 2. Якщо rank = P/2-1, то передати MO, Z(k*H), E задачі
					// знизу.
					MPI.COMM_WORLD.Isend(box, 0, 1, MPI.OBJECT, P - 1,
							KEY.SEND_RECIVE_MO_Z_E_TO_P_1);
			...
			MPI.COMM_WORLD.Recv(box, 0, 1, MPI.OBJECT, leftRank,
					KEY.SEND_RECIVE_B_MK_MR_alfa);
			...
			if (rank == P - 1 && notTwoCore) {
				// 4. Якщо rank = P-1, то прийняти MO, Z(k*H), E від задачі
				// зверху.
				MPI.COMM_WORLD.Recv(box, 0, 1, MPI.OBJECT, k - 1,
						KEY.SEND_RECIVE_MO_Z_E_TO_P_1);
				MO = box[0].getMatrix(0);
				E = box[0].getVector(0);
				Z = box[0].getVector(1);
		...
		if (rank > endFirsRowRank) {
			...
			if (rank == minRank) {
				...
				if (notFourCore && notTwoCore) {
					// 14. Якщо rank = (k-1)/2, то Отримати minZright від задачі
					// справа
					MPI.COMM_WORLD.Recv(res, 0, 1, MPI.INT, rank + 1, 104);
					// 15. Якщо rank = (k-1)/2, то обчислити minZ = (minZ,
					// minZleft, minZright)
